"""
Script for automating the generation of batch files for usersort at OCL.
Why do it easily when you can automate it?

Python 2

Collects all W186 in one root file, and all 60Ni in one. OR if separate, you need a command line input separately.
"""
...
# Name of batchfile. eg. batchname_W.batch
batchname = "W186_autobatch"

# eg. outputname_no/W .root
outputfilename = "W186_autoroot"
...
if collected:
    ...
    j = 0
    for i in ["W186", "Ni60"]:
        export_str = "export root " + outputfilename + i + ".root"

        #metadata = open("fileinfo.txt", "a+")
        #metadata.write(filenames[i] + " ---> " + outputfilename + ".root")
        #metadata.close()

        batchfile = open("%s%s.batch"%(batchname,i), "w+") # write and create new 
        batchfile.write(main_str_1)
        batchfile.write(filenames[j])
        batchfile.write(main_str_2)
        batchfile.write(export_str)
        batchfile.close()
        print "Created file output W186_autobatch_collected_sirius_%s.batch"%i
        j+=1



if separate:
    filenames = ["""data file sirius-20160420-135618.data""",
    """data file sirius-20160420-171400.data""", 
    """data file sirius-20160420-182451.data""",
    """data file sirius-20160420-233716.data""",
    """data file sirius-20160421-091709.data""",
    ...
    try:
        ...
        sys.exit("Error. Got no input on which file to run. Len(filenames) = %d"% len(filenames))
    export_str = """
    export root """ + outputfilename + "%03d"%i + ".root"

    metadata = open("fileinfo.txt", "a+")
    metadata.write(filenames[i] + " ---> " + outputfilename + ".root")
    metadata.close()

    batchfile = open("%s%03d.batch"%(batchname, i), "w+") # write and create new 
    batchfile.write(main_str_1)
    batchfile.write(filenames[i])
    batchfile.write(main_str_2)
    batchfile.write(export_str)
    batchfile.close()
