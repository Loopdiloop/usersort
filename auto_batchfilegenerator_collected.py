...
Collects all W186 in one root file, and all 60Ni in one.
"""
...
j = 0
for i in ["W186", "Ni60"]:
    outputfilename = "W186_autogenerated_collected_%s"%i
    export_str = "export root " + outputfilename + ".root"

    #metadata = open("fileinfo.txt", "a+")
    #metadata.write(filenames[i] + " ---> " + outputfilename + ".root")
    #metadata.close()

    batchfile = open("W186_autobatch_collected_sirius_%s.batch"%i, "w+") # write and create new 
